...
const ContactForm: React.FC = () => {
  ...
  const { isLoading, error, isSubmitted } = useSelector((state: RootState) => state.contact);

  const [submitContactForm, { isLoading: mutationLoading, error: mutationError }] =
  ...
  return (
    <div className="max-w-2xl mx-auto mt-12 p-6 bg-white rounded-lg shadow-2xl">
      <h1 className="text-2xl font-bold mb-6 text-center">Contact Us</h1>
      <h3 className="font-bold text-center mb-12">Have a question or feedback? We’re here to help!</h3>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="mb-7">
          ...
          <input
            ...
            className={`mt-1 block w-full px-3 py-2 border font-semibold ${
              errors.name ? "border-red-500" : "border-gray-300"
            } rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm`}
            {...register("name", { required: "Name is required" })}
            disabled={isLoading || mutationLoading} // Disable during submission
          />
          {errors.name && (
        ...
        <button
          type="submit"
          className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#7BBFB3] hover:bg-teal-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 disabled:opacity-50 disabled:cursor-not-allowed"
          disabled={isLoading || mutationLoading} // Disable during submission
        >
