    ...
    return (
        <div className="max-w-md mx-auto mt-12 p-8 bg-white rounded shadow">
            ...
            <form className={`${epilogue.className}`} onSubmit={handleSubmit(onSubmit)} noValidate>
                ...
                {serverError && (
                    <p className={`text-sm mb-4 ${serverError.includes('successfully') ? 'text-green-500' : 'text-red-500'}`}>
                        {serverError}
                    </p>
                )}
            ...
            <p className="mt-6 text-gray-600 text-center">
                <Link href="/login" className="text-[#7BBFB3] font-semibold">Back to Login</Link>
            </p>
        </div>
    );
}