...
export async function GET(req: NextRequest, context: { params: { id: string } }) {
  ...
  try {
    ...
    if (backend) {
      try {
        const r = await fetch(`${backend}/api/v1/cv/${encodeURIComponent(id)}`, {
          method: "GET",
          headers: {
    ...
    const mock = {
      ...
      suggestions: [
        { id: "s1", type: "quantification", content: "Add measurable outcomes to your recent role.", applied: false },
        { id: "s2", type: "missing_keywords", content: "Consider adding Docker and CI/CD if applicable.", applied: false },
      ],
      score: 78,
